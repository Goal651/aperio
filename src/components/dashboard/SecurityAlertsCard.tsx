    ...
    <div className="glass-card p-6 md:p-8 animate-fade-in group relative overflow-hidden h-full flex flex-col" style={{ animationDelay: "0.1s" }}>
      ...
      <div className="flex items-center justify-between mb-8 relative z-10">
        <div className="flex items-center gap-4">
          <div className="rounded-2xl bg-destructive/10 p-3 border border-destructive/20 shadow-sm group-hover:scale-110 transition-transform duration-500">
            <AlertTriangle className="h-6 w-6 text-destructive" />
          </div>
          <div>
            <h3 className="font-black text-foreground tracking-tight">Security Alerts</h3>
            <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground opacity-60">Real-time Vulnerability Indexed</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4 flex-1 relative z-10">
        {loading ? (
          <>
            {[0, 1, 2].map((i) => (
              <div key={i} className="flex items-center justify-between rounded-2xl bg-secondary/10 p-5 border border-border/40">
                <div className="flex items-center gap-4">
          ...
          <div
            key={alert.label}
            className="flex items-center justify-between rounded-2xl bg-secondary/10 p-5 border border-border/40 hover:bg-secondary/20 hover:border-primary/20 transition-all group/item"
          >
            <div className="flex items-center gap-4">
              <div className={`p-2.5 bg-background rounded-xl border border-border/40 shadow-sm transition-transform group-hover/item:scale-110 ${alert.color}`}>
            ...
            <div className="flex items-center gap-2">
              {alert.critical > 0 && (
                <div className="bg-destructive/10 text-destructive border border-destructive/20 px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest animate-pulse shadow-sm">
                    {alert.critical} Crit
                </div>
              )}
              {alert.high > 0 && (
                <div className="bg-warning/10 text-warning border border-warning/20 px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest shadow-sm">
                    {alert.high} High
                </div>
              )}
              {alert.medium > 0 && (
                <div className="bg-primary/10 text-primary border border-primary/20 px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest shadow-sm">
                    {alert.medium} Med
                </div>
              )}
              {alert.critical === 0 && alert.high === 0 && alert.medium === 0 && (
                <div className="flex items-center gap-2 px-3 py-1 bg-success/5 border border-success/20 rounded-lg">
                    ...
                    <span className="text-[9px] font-black uppercase tracking-widest text-success/60">Protected</span>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="mt-8 pt-6 border-t border-border/40 relative z-10 flex items-center justify-between">
        <div className="flex items-center gap-4">
            <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground opacity-60">Total Findings</p>
                <p className="text-xl font-black text-foreground leading-none mt-1">{state.alerts?.length || 0}</p>
            </div>
            <div className="h-8 w-px bg-border/40" />
            <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground opacity-60">Critical</p>
                <p className="text-xl font-black text-destructive leading-none mt-1">{state.alerts?.filter(a => a.severity === 'critical').length || 0}</p>
            </div>
        </div>
        <Button variant="ghost" size="sm" className="h-10 px-4 rounded-xl gap-2 text-[10px] font-black uppercase tracking-widest text-muted-foreground hover:text-primary hover:bg-primary/5 transition-all">
            Full Audit
