    ...
    <div className="glass-card p-6 md:p-8 animate-fade-in group relative overflow-hidden" style={{ animationDelay: "0.15s" }}>
      ...
      <div className="flex items-center justify-between mb-8 relative z-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="font-semibold text-foreground tracking-tight">Security Score</h3>
            <p className="text-[10px] font-semibold uppercase  text-muted-foreground opacity-60">Aggregate Infrastructure Health</p>
          </div>
        </div>
        <div className="p-2 bg-secondary/20 rounded hover:bg-secondary/30 cursor-help transition-colors">
        ...
        <div className="flex flex-col items-center justify-center py-4">
          <div className="relative h-48 w-48 group/chart">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none group-hover/chart:scale-110 transition-transform duration-500">
              <span className={`text-5xl font-semibold tracking-tighter ${getScoreColor()}`}>{score}</span>
              <span className="text-[10px] font-semibold uppercase  text-muted-foreground opacity-60 mt-1">{getScoreLabel()}</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="mt-8 pt-6 border-t border-border/40 grid grid-cols-2 gap-4 relative z-10">
        <div className="p-3 bg-secondary/10 rounded border border-border/40 hover:bg-secondary/20 transition-all">
          <div className="flex items-center gap-2 mb-1">
            <TrendingUp className="h-3 w-3 text-success" />
            <span className="text-[9px] font-semibold uppercase  text-muted-foreground opacity-60">Status</span>
          </div>
          <p className="text-sm font-semibold text-foreground">{score}% Health</p>
        </div>
        <div className="p-3 bg-secondary/10 rounded border border-border/40 hover:bg-secondary/20 transition-all">
          <div className="flex items-center gap-2 mb-1">
            <Zap className="h-3 w-3 text-amber-400" />
            <span className="text-[9px] font-semibold uppercase  text-muted-foreground opacity-60">Control</span>
          </div>
          <p className="text-sm font-semibold text-foreground">{12}/{15} Ops</p>
        </div>
      </div>
    </div>
  );
